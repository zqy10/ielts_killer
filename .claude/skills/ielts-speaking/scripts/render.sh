#!/usr/bin/env bash
# 编译 ielts-speaking/speaking.tex → speaking.pdf
# 用法: .claude/skills/ielts-speaking/scripts/render.sh [--no-open]
set -euo pipefail

# 脚本位于 .claude/skills/ielts-speaking/scripts/，项目根在上面 4 层
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
WORK="$ROOT/ielts-speaking"
TEX="$WORK/speaking.tex"
OUT="$WORK/speaking.pdf"
BUILD="$WORK/.build"
AUTO_OPEN=true
