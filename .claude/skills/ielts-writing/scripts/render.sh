#!/usr/bin/env bash
# 编译 ielts-writing/writing.tex → writing.pdf
# 用法: .claude/skills/ielts-writing/scripts/render.sh [--no-open]
set -euo pipefail

# 脚本位于 .claude/skills/ielts-writing/scripts/，项目根在上面 4 层
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
WORK="$ROOT/ielts-writing"
TEX="$WORK/writing.tex"
OUT="$WORK/writing.pdf"
BUILD="$WORK/.build"
AUTO_OPEN=true
